...
# --- ИЗМЕНЕНО: Добавляем date из datetime ---
from datetime import date, datetime, UTC # <-- Добавили date и UTC
...
async def recalculate_and_save_goal(user_id: int, state: FSMContext):
    ...
    # --- ИЗМЕНЕНИЕ: Сохраняем норму и добавляем запись в историю ---
    try:
        # 1. Обновляем текущую норму в таблице users
        await db.update_user_daily_goal(db.db_pool, user_id, calories)

        # 2. Добавляем/обновляем запись в goal_history на СЕГОДНЯШНЮЮ ДАТУ (UTC)
        # Важно использовать UTC дату для консистентности effective_date
        today_utc_date = datetime.now(UTC).date()
        await db.add_goal_history_entry(db.db_pool, user_id, today_utc_date, calories)

        logger.info(f"Норма калорий для {user_id} пересчитана ({calories} ккал) и сохранена (в users и goal_history на {today_utc_date}).")
...
async def show_settings_menu(message_or_callback: Message | CallbackQuery, state: FSMContext):
    ...
    if isinstance(message_or_callback, Message): answer_method = message_or_callback.answer
    else: answer_method = message_or_callback.message.answer; await message_or_callback.answer()

    # Получаем данные профиля
    profile_data = None; current_goal_text = "Не установлена"; current_gender_text = "Не указан"
    current_height_text = "Не указан"; current_weight_text = "Не указан"; current_norm_text = "Не рассчитана"
    ...
    if db.db_pool:
        ...
        if profile_data:
            ...
            height = profile_data.get('height'); weight = profile_data.get('current_weight')
            ...
            else: # Пробуем пересчитать, если нормы нет, но данные есть
                if all([weight, height, gender, goal]):
                     norm = await recalculate_and_save_goal(user_id, state) # Пересчет вызовет и запись в историю
                     if norm: current_norm_text = f"~<b>{norm}</b> ккал/день"
                     else: current_norm_text = "Ошибка расчета"
                else: current_norm_text = "Не рассчитана (заполните профиль)"
    ...
    if isinstance(message_or_callback, CallbackQuery):
         with suppress(TelegramBadRequest):
              ...
              await state.set_state(Settings.waiting_for_action); return
...
async def handle_settings_command(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id; logger.info(f"Пользователь {user_id} вызвал /settings.")
...
async def handle_setweight_command(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id; logger.info(f"Пользователь {user_id} вызвал /setweight.")
...
async def handle_settings_action(callback: CallbackQuery, state: FSMContext):
    ...
    action = callback.data.split(":")[1]; user_id = callback.from_user.id; message = callback.message
    with suppress(TelegramBadRequest): await message.edit_reply_markup(reply_markup=None) # Убираем кнопки
    ...
    if action == "change_goal":
        ...
        await state.set_state(Settings.waiting_for_action) # Ждем нажатия инлайн-кнопки цели
        ...
        await state.clear(); await message.answer("Настройки закрыты.", reply_markup=main_action_keyboard()); await handle_today(message)
...
async def handle_goal_selection(callback: CallbackQuery, state: FSMContext):
    ...
    goal = callback.data.split(":")[1]; user_id = callback.from_user.id; message = callback.message
    with suppress(TelegramBadRequest): await message.edit_reply_markup(reply_markup=None)
    goal_to_save = goal if goal != "none" else None; goal_text = goal
    if db.db_pool:
        ...
        if success:
            ...
            await recalculate_and_save_goal(user_id, state) # Пересчитываем норму и записываем историю
            await message.answer(f"✅ Цель обновлена!")
            await show_settings_menu(callback, state) # Показываем обновленное меню
        else: await message.answer("Не удалось обновить цель."); await show_settings_menu(callback, state)
    else: await message.answer("Ошибка подключения к БД."); await state.clear(); await handle_today(message)
    await callback.answer()
...
async def handle_gender_selection(callback: CallbackQuery, state: FSMContext):
    ...
    gender = callback.data.split(":")[1]; user_id = callback.from_user.id; message = callback.message
    with suppress(TelegramBadRequest): await message.edit_reply_markup(reply_markup=None)
    if db.db_pool:
        ...
        else: await message.answer("Не удалось обновить пол."); await show_settings_menu(callback, state)
    else: await message.answer("Ошибка подключения к БД."); await state.clear(); await handle_today(message)
    await callback.answer()
...
async def process_height_input(message: Message, state: FSMContext):
    ...
    try: height = int(message.text.strip()); assert 100 <= height <= 250
    except (ValueError, AssertionError): await message.reply("Введите рост целым числом от 100 до 250 см. Или /cancel"); return
    if db.db_pool:
        ...
        if success:
            ...
            await show_settings_menu(message, state) # Возвращаемся в меню
        else: await message.answer("Не удалось обновить рост.", reply_markup=ReplyKeyboardRemove()); await show_settings_menu(message, state)
    else: await message.answer("Ошибка подключения к БД.", reply_markup=ReplyKeyboardRemove()); await state.clear(); await handle_today(message)
...
async def process_weight_input(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    try: weight_str = message.text.strip().replace(',', '.'); weight = float(weight_str); assert 30.0 <= weight <= 300.0
    except (ValueError, AssertionError): await message.reply("Введите вес числом от 30 до 300 кг (например, 75.5). Или /cancel"); return
    if db.db_pool:
        ...
        if success:
            ...
            await show_settings_menu(message, state) # Возвращаемся в меню
        else: await message.answer("Не удалось обновить вес.", reply_markup=ReplyKeyboardRemove()); await show_settings_menu(message, state)
    else: await message.answer("Ошибка подключения к БД.", reply_markup=ReplyKeyboardRemove()); await state.clear(); await handle_today(message)

# --- Обработчик отмены для состояний ввода ---
...
async def cancel_settings_input_handler(message: Message, state: FSMContext):
    ...
    await show_settings_menu(message, state) # Возвращаемся к главному меню настроек
