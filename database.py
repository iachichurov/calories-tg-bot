...
async def create_tables_if_not_exist(pool: asyncpg.Pool):
    ...
    async with pool.acquire() as connection:
        async with connection.transaction():
            # --- ИЗМЕНЕНО: Обновляем структуру users ---
            await connection.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    -- Данные для расчета нормы
                    current_weight REAL, -- Текущий вес (может быть дробным)
                    height INTEGER, -- Рост в см
                    gender VARCHAR(10), -- 'male' / 'female'
                    goal VARCHAR(20), -- 'deficit' / 'maintenance' / 'surplus'
                    -- Убираем birth_date, activity_level
                    -- daily_calorie_goal теперь хранит норму, рассчитанную по LBM/БЖУ
                    daily_calorie_goal INTEGER
                );
            """)
            # Таблица user_products (без изменений)
            await connection.execute("""
            ...
            """)
            # Таблица food_entries (без изменений)
            await connection.execute("""
            ...
            """)
            # Функция и триггер для updated_at (без изменений)
            await connection.execute("""
...
async def add_or_update_user(pool: asyncpg.Pool, user_id: int, first_name: str | None, last_name: str | None, username: str | None):
    ...
    # Не обновляем здесь данные профиля (вес, рост и т.д.)
    sql = """
    ...
    async with pool.acquire() as connection:
        try:
            ...
            # Возвращаем флаг, был ли пользователь новым
            return is_new_user
        except Exception as e:
            logger.error(f"Ошибка при добавлении/обновлении пользователя {user_id}: {e}", exc_info=True)
            return False # В случае ошибки считаем, что не новый

# --- НОВАЯ ФУНКЦИЯ ---
async def get_user_profile_data(pool: asyncpg.Pool, user_id: int) -> Optional[asyncpg.Record]:
    """Получает данные профиля пользователя (вес, рост, пол, цель) для расчета нормы."""
    sql = "SELECT current_weight, height, gender, goal FROM users WHERE user_id = $1;"
    async with pool.acquire() as connection:
        try:
...
async def update_user_profile_field(pool: asyncpg.Pool, user_id: int, field: str, value: Any) -> bool:
    """Обновляет одно поле профиля пользователя (например, вес, рост, пол, цель)."""
    # Базовая валидация имени поля для предотвращения SQL-инъекций в имени столбца
    ...
    # Используем f-строку для имени поля, но параметризацию для значения
...
# --- Переименована, т.к. обновляет только одно поле ---
async def update_user_timezone_db(pool: asyncpg.Pool, user_id: int, timezone: str):
    ...
    # Вызываем общую функцию обновления
    await update_user_profile_field(pool, user_id, "timezone", timezone)

# --- НОВАЯ ФУНКЦИЯ для обновления рассчитанной нормы ---
...
# --- Функции отчетов (без изменений в этой итерации) ---
...
async def get_last_n_days_entries(pool: asyncpg.Pool, user_id: int, tz_name: str, days: int = 7) -> List[asyncpg.Record]:
    ...
    end_date_exclusive_local = today_start_local + timedelta(days=1)
...
async def get_current_month_entries(pool: asyncpg.Pool, user_id: int, tz_name: str) -> List[asyncpg.Record]:
    ...
    start_date_local = datetime(current_date_local.year, current_date_local.month, 1, tzinfo=user_tz)
